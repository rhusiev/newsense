...
async def main():
    ...
    try:
        ...
        if not art1:
            print(f"❌ Could not find an article matching '{term1}'")
            return
        print(f"1️⃣  Found: '{art1['title'][:60]}...'")
        if not art1["cluster_id"]:
            print("   ⚠️  This article has not been clustered yet.")
            return

        if not art2:
            print(f"❌ Could not find an article matching '{term2}'")
            return
        ...
        if art1["cluster_id"] == art2["cluster_id"]:
            print("✅ EXACT MATCH: Both articles are in the SAME cluster.")
