...
async fn fetch_feed_with_count(db: &PgPool, feed_id: Uuid) -> Result<FeedResponse, sqlx::Error> {
    sqlx::query_as!(
        FeedResponse,
        r#"
...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/feeds/popular", get(list_popular_feeds))
...
async fn add_feed(
    ...
    let existing_feed = sqlx::query!(
        "SELECT f.id FROM feeds f WHERE f.owner_id = $1 AND f.source_id = $2",
        user_id,
        source.id
    )
    ...
    if let Some(f) = existing_feed {
        let feed = fetch_feed_with_count(&state.db, f.id).await.map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse {
                    error: e.to_string(),
    ...
    tokio::spawn(async move {
        let url = format!("http://fetcher:3003/feeds/{}/refresh", new_feed_id);
        client.post(&url).send().await.ok();
    });

    let feed = fetch_feed_with_count(&state.db, new_feed_id)
        .await
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
